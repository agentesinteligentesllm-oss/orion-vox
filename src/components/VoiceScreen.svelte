...
import {
  ...
  executePlan,
} from '../lib/api/execute-plan-client.ts';
import {
...
import type { AuditMirrorEntry, Idioma } from '../lib/storage/types.ts';
import {
...
function buildExecuteResultSummary(
  plan: { operation: string; table: string },
  rowsAffected: number,
  dryRun: boolean,
): string {
  ...
  const t = plan.table;
  if (dryRun) {
    return `Simulación: se afectarían ${n} ${n === 1 ? 'registro' : 'registros'} en ${t}.`;
  }
  switch (plan.operation) {
    case 'select':
      return `Listo. ${n === 1 ? 'Encontré 1 registro' : `Encontré ${n} registros`} en ${t}.`;
    case 'insert':
      return `Listo. Registro creado en ${t}.`;
    case 'update':
      return `Listo. ${n === 1 ? 'Se actualizó 1 registro' : `Se actualizaron ${n} registros`} en ${t}.`;
    case 'delete':
      return `Listo. ${n === 1 ? 'Se eliminó 1 registro' : `Se eliminaron ${n} registros`} en ${t}.`;
    default:
      return 'Listo. Operación completada.';
  }
}

async function handleConfirmed(): Promise<void> {
  if (planResponse?.kind !== 'plan') return;
  const plan = planResponse.plan;
  const schemaHash = planResponse.schema_hash;
  const token = authStore.session?.access_token;
  if (!token) {
    planResponse = null;
    router.navigate('config', { firstTime: true });
  ...
  // Cerrar modal y mostrar spinner inmediatamente
  planResponse = null;
  planError = null;
  voiceState = 'processing';

  const dryRun = (await localStore.getSetting<boolean>('dryRun')) ?? false;
  ...
  try {
    ...
    const summary = buildExecuteResultSummary(plan, result.rows_affected, dryRun);

    const mirrorEntry: AuditMirrorEntry = {
      id: result.audit_id,
      ts: new Date().toISOString(),
      user_prompt: lastResult,
      plan_json: plan,
      error: null,
      rows_affected: result.rows_affected,
      result_summary: summary,
      server_audit_id: result.audit_id,
    };
    await localStore.appendAuditMirror(mirrorEntry);

    tts.speak(summary);
  } catch (err) {
    const clientErr =
      err instanceof ExecutePlanClientError
        ? err
        : new ExecutePlanClientError({ code: 'internal' });
    ...
    const mirrorEntry: AuditMirrorEntry = {
      ...
      server_audit_id: clientErr.auditId,
    };
    await localStore.appendAuditMirror(mirrorEntry);

    planError = clientErr.messageEs;
    tts.speak(`Hubo un error. ${clientErr.messageEs}`);

    const isAuthErr =
      clientErr.code === 'unauthorized' ||
      clientErr.code === 'invalid_token' ||
      clientErr.code === 'missing_token';
    if (isAuthErr) {
      router.navigate('config', { firstTime: true });
    } else if (clientErr.code === 'schema_stale') {
